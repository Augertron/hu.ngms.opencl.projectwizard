 ...
 */
public class OpenCLConfigWizardPage extends WizardPage {

    public static final String PAGE_ID = "org.eclipse.cdt.managedbuilder.ui.wizard.CConfigWizardPage"; //$NON-NLS-1$

    private static final Image IMG_CONFIG = ManagedBuilderUIImages
	    .get(ManagedBuilderUIImages.IMG_BUILD_CONFIG);
    ...
    private OpenCLWizardHandler handler;
    public boolean pagesLoaded = false;
    private IToolChain[] visitedTCs = null;
    IWizardPage[] customPages = null;

    private String message = Messages.OpenCLConfigWizard_message;

    public OpenCLConfigWizardPage(OpenCLWizardHandler h) {
	super(Messages.OpenCLProjectWizard_project);
	...
	setupLabel(c1, Messages.OpenCLConfigWizard_projectType,
		GridData.BEGINNING);
	l_projtype = setupLabel(c1, EMPTY_STR, GridData.FILL_HORIZONTAL);
	setupLabel(c1, Messages.OpenCLConfigWizard_toolchains,
		GridData.BEGINNING);
	l_chains = setupLabel(c1, EMPTY_STR, GridData.FILL_HORIZONTAL);
	setupLabel(c1, Messages.OpenCLConfigWizard_configurations,
		GridData.BEGINNING);
	...
	b1.setText(Messages.OpenCLConfigWizard_selectAll);
	...
	b2.setText(Messages.OpenCLConfigWizard_deselectAll);
	...
	b3.setText(Messages.OpenCLConfigWizard_advanced);
	...
	lb.setText(Messages.OpenCLConfigWizard_comment);

	setControl(parent);
    }

    static public CfgHolder[] getDefaultCfgs(OpenCLWizardHandler handler) {
	...
	for (IToolChain tc : handler.getSelectedToolChains()) {
		...
		cfgs = CfgHolder.cfgs2items(ManagedBuildManager
			.getExtensionConfigurations(tc,
				OpenCLWizardHandler.ARTIFACT, id));
	    else if (pt != null)
		cfgs = CfgHolder.cfgs2items(ManagedBuildManager
		...
		if (cfgs[j].isSystem()
			...
			// Bug 335338 don't include project type configurations
			// if we're not creating a project-typed project
			// Project types are non-default if their nameAttribute
			// != "" (see: ManagedBuildWizard:
			// "old style project types")
			|| (pt == null
				&& cfgs[j].getConfiguration() != null
				&& cfgs[j].getConfiguration().getProjectType() != null && cfgs[j]
				.getConfiguration().getProjectType()
				.getNameAttribute().length() > 0))
		    continue;
		out.add(cfgs[j]);
	    }
	}
    ...
    /**
     * Checks whether we've already worked with given set of selected toolchains
     *
	...
	errorMessage = null;
	message = Messages.OpenCLConfigWizard_message;
	return true;
    }
	...
	return Messages.OpenCLConfigWizard_title;
    }
